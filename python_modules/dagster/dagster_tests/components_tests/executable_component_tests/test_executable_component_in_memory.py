...
def asset_in_component(
    component: ExecutableFunctionComponent, key: CoercibleToAssetKey
) -> AssetsDefinition:
...
def test_include() -> None:
    assert ExecutableFunctionComponent
...
def test_basic_singular_asset() -> None:
    ...
    component = ExecutableFunctionComponent(
        execution=ExecutionSpec(name="op_name", fn=_execute_fn),
        assets=[AssetSpec(key="asset")],
    )

    assert isinstance(component, ExecutableFunctionComponent)
...
def test_basic_singular_asset_with_callable() -> None:
    ...
    component = ExecutableFunctionComponent(execution=op_name, assets=[AssetSpec(key="asset")])

    assert isinstance(component, ExecutableFunctionComponent)
    assert_singular_component(component)


def assert_singular_component(component: ExecutableFunctionComponent) -> None:
    defs = component.build_defs(ComponentLoadContext.for_test())
...
def test_local_import() -> None:
    ...
    with scaffold_defs_sandbox(component_cls=ExecutableFunctionComponent) as sandbox:
        ...
        with sandbox.load(
            component_body={
                "type": "dagster.components.lib.executable_component.component.ExecutableFunctionComponent",
                "attributes": {
                    "execution": {
                        "name": "op_name",
